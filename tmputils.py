...
def constructFeaturePyramidForImg(img, params, levels = 0, ds={}, cutout_name='', hog=False):
    
    try:
        model = ds['resnet_model']
    except KeyError:
        if hog==False:
    ...
    if len(scales) == 1:
        ...
        t_img = torch.tensor(I1, requires_grad=False)
        t_img = t_img.permute(2,0,1)
        t_img = t_img.unsqueeze(0).to(torch.float32)
        if torch.cuda.is_available():
            t_img = t_img.to(torch.cuda.current_device())
        with torch.no_grad():
            out = model_med.forward(t_img)
        rows, cols = (out.shape[2], out.shape[3])
            
        feat_1k = lin.forward(out.squeeze())
        canoSize = {'nrows' : rows, 'ncols' : cols}
        pyramid = {'features' : {0 : feat_1k}, 'scales' : scales, 'canonicalScale' : canoScale, 
               'sbins' : sBins, 'canonicalSize' : 0, 'gradimg' : 0}
        
        return pyramid
    ### Make sure you never resize the image more than once
    
    if model:
        if cutout_name in ds['saved_resizes']:
            big_img = ds['saved_resizes'][cutout_name]
        else:
            big_img = cv.resize(img, (img.shape[0] * 2, img.shape[1] * 2))
            ds['saved_resizes'][cutout_name] = big_img

        
    ### If we are extracting features for a patch we choose to represent this
    ### as a 1k vector. If we want to have 512 vector extract it with this:
    ### torch.nn.Sequential(*(list(ds['resnet_type'].children())[:-1]))
    ...
    for idx, s in enumerate(scales):
        ...
        #I1 = np.round(I1, 8)
            big_img = img_as_float(big_img)
            
            if not fast_resize:
                big_I1 = imresize_like_matlab.imresize(big_img, canoScale/s)
            
            else:        
                big_I1cols, big_I1rows = imresize_like_matlab.speedSizeFromScale(np.array(big_img.shape)[:2], canoScale/s)
                big_I1 = cv.resize(big_img, (big_I1rows, big_I1cols))
    
            big_nrows, big_ncols, _ = big_I1.shape
        
        ## No need to clip because sBins is 1 with resnet
        
        
        
        #### RESNET
            out, rows, cols  = extract_features(model, big_I1)
            gc.collect()
        
        
            #t_img = torch.tensor(big_I1, requires_grad=False)
            #t_img = t_img.permute(2,0,1)
            #t_img = t_img.unsqueeze(0).to(torch.float32)
            #if torch.cuda.is_available():
            #    t_img = t_img.to(torch.cuda.current_device())
        ...
        if model:
            #with open(os.path.join(ds['raw_outdir'], 'pyr_tmp.pickle'), 'wb') as h:
            #    pickle.dump(out, h)
            #del out
            
            #out = pickle.load(open(os.path.join(ds['raw_outdir'], 'pyr_tmp.pickle'), 'rb'))
            pyramidLevs[idx] = np.moveaxis(out, 0, 2)
            #### save out to disk
...
def extract_features(model, inp):
    
    t_img = torch.tensor(inp, requires_grad=False)
    t_img = t_img.permute(2,0,1)
    t_img = t_img.unsqueeze(0).to(torch.float32)
    if torch.cuda.is_available():
        t_img = t_img.to(torch.cuda.current_device())
...
#@jit(nopython=True)
def unentanglePyramid(pmid, params, ds):
    
    if ds['resnet_model']:
        prSize, pcSize, pzSize, nExtra = (1,1,512,0)
...
def getimg(ds, idx, integer_pixels=False):
    ...
    try:
        ...
        print(file, cutout_path, img_path)
        raise TypeError
...
def RAM_usage():
    ...
    print('Total:', psutil.virtual_memory().total >> 20)
    print('Available:', psutil.virtual_memory().available >> 20)
    print('Percent:', psutil.virtual_memory().percent)
    print('Used:', psutil.virtual_memory().used >> 20)
    print('Free:', psutil.virtual_memory().free >> 20)
    print('Active:', psutil.virtual_memory().active >> 20)
    print('Inactive:', psutil.virtual_memory().inactive >> 20)
    print('Wired:', psutil.virtual_memory().wired >> 20)
