...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 13 13:01:04 2022
...
def stitcher_to_array(args, img):
    ...
    stitched_array = stitched_array[0:720, 0:4260]
...
def seq_24(args, image, base_name, output_dir, gps):
    ...
    for i in pitch:
        for j in yaw:
            ...
            x = (j - 1) * 320
            w = 640
            cropped_img = cutter(stitched_array, y, h, x, w)             
            
            #cropped_img = cropped_img[:, :, [2, 1, 0]]
            
            
            name = base_name + '_pitch' + str(i) + '_yaw' + str(j)
            ...
            with open('groundtruth_gps.csv', 'a') as f:
                csv_writer = csv.writer(f, delimiter=',')
                csv_writer.writerow([csv_filename, gps[0], gps[1]])
...
def download(args, api_url, image_url, directory, full_dir):
    ...
    try:
        ...
        seq_24(args, image, id_of_pano, full_dir, gps)
...
class Task(object):
    def __init__(self, args, api_url, image_url, directory, full_dir):
        #print('Task init')
    ...
    def __call__(self):
        #print('Task call')
        download(self.args, self.api_url, self.image_url, self.dir, self.top_dir)
...
def main(args):
    ...
    for api_url, image_url in zip(urls, image_urls):
        
        tasks.put(Task(args, api_url, image_url, directory, full_directory))
